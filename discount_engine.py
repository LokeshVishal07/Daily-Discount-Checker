...
import numpy as np
import pandas as pd
from config.settings import EXCLUSION_RULES
...
def _calc_discounts(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Fallback: no RRP from ZeCom → use marketplace listed price
    ...
    # How much discount below SRP (used for EXCLUDE rule check)
...
def _apply_flags(df: pd.DataFrame) -> pd.DataFrame:
    ...
    results = [_eval_remark(r.get("remark",""), r.get("seller_disc_pct",0) or 0,
...
def _eval_remark(remark: str, seller_pct: float, srp_pct: float):
    """Returns (rule_label, max_pct|None, is_flagged, reason, severity)."""
    ...
    return (f"Remark: {remark[:60]}", None, False,
            f"Unrecognised remark '{remark[:60]}' — manual review.", "grey")


# ── Aggregation helpers ────────────────────────────────────────────────────────
def summary_by_marketplace(df: pd.DataFrame) -> pd.DataFrame:
    return (
        df.groupby(["region", "marketplace"])
        .agg(
...
def exclusion_summary(df: pd.DataFrame) -> pd.DataFrame:
    """
    Per-exclusion-remark breakdown:
        Sum of RRP  |  Sum of paid_price  |  Total Discount Amount  |  Total Discount %
    across all marketplaces.
    """
    grp = df.groupby(["region", "marketplace", "allowed_rule", "flag_severity"]).agg(
...
def flagged_orders(df: pd.DataFrame) -> pd.DataFrame:
    keep = [c for c in [
        ...
        "order_status","order_date","rrp_used","srp_used","paid_price",
