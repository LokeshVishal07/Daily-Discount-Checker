"""
app.py — Daily Discount Checker v2
...
from utils.zecom_loader import (
...
from utils.content_loader import load_content_file
from utils.order_loader import load_order_file
from utils.discount_engine import (
    run_pipeline, summary_by_marketplace, exclusion_summary, flagged_orders,
)
from utils.exporter import build_report
...
<style>
  ...
  .pill{display:inline-block;padding:1px 8px;border-radius:12px;
    font-size:.72rem;font-weight:700;color:#fff;margin:1px;}
...
with st.sidebar:
    ...
    st.caption("Select the region(s) you are responsible for today.")
    active_regions = st.multiselect(
        "Your region(s)", options=REGIONS, default=["MY"],
    ...
    # Region → PIC label (purely for display/download naming)
    st.markdown("**PIC name (optional — for report labelling)**")
    pic_names = {}
    for r in active_regions:
        pic_names[r] = st.text_input(f"{r} PIC", placeholder=f"e.g. Ahmad / Priya",
                                      key=f"pic_{r}", label_visibility="collapsed")

    # ── Content file ──────────────────────────────────────────────────────────
    st.divider()
    st.markdown("### 📄 Content File")
    st.caption("Master EAN → Article Number mapping (upload once per session)")
    ...
    st.caption("Upload your ZeCom file(s). The app auto-detects the header row — works even after bi-weekly column changes.")
        ...
        for zf in z_files:
            ...
            for region in active_regions:
                ...
                st.success(f"✅ ZeCom {region} — {len(df_tab):,} rows, "
                           f"{len(num_cols)} numeric cols, {len(txt_cols)} text cols")

    # ── Per-region column mapping ─────────────────────────────────────────────
    if st.session_state["zecom_data"]:
        st.divider()
        st.markdown("### 🗂️ Column Mapping")
        st.caption("Columns are auto-suggested. Change as needed — works even if ZeCom columns shifted.")
        ...
        for region in active_regions:
            ...
            tc   = zdata["txt_cols"]
            ac   = zdata["all_cols"]
            rclr = REGION_COLORS.get(region, "#333")

            with st.expander(f"**{region}** — column mapping", expanded=True):
                # Article / Style# column
                art_default = guess_article_col(ac)
                art_col = st.selectbox(
                    f"{region}  Article / Style# column",
                    options=ac,
                    index=_idx(ac, art_default),
                    key=f"art_{region}",
                    help="The column containing article / style numbers (e.g. Style#, PIM Article#). "
                         "Changes between file versions are handled automatically.",
                )

                # RRP
                rrp_default = guess_rrp_col(nc, region)
                rrp_col = st.selectbox(
                    f"{region}  RRP column",
                    options=nc,
                    index=_idx(nc, rrp_default),
                    key=f"rrp_{region}",
                    help="Retail Recommended Price. All numeric columns shown — pick the right one.",
                )

                # SRP / MD Price
                srp_default = guess_srp_col(nc, region)
                srp_col = st.selectbox(
                    f"{region}  SRP / MD Price column",
                    options=["(same as RRP — no separate SRP)"] + nc,
                    index=_idx(["(same as RRP — no separate SRP)"] + nc, srp_default),
                    key=f"srp_{region}",
                    help="SRP or markdown price. Used as the ceiling for EXCLUDED products. "
                         "If no separate SRP exists, select '(same as RRP)'.",
                )
                srp_col = None if srp_col == "(same as RRP — no separate SRP)" else srp_col

                # MP Remarks / Exclusion
                rmk_default = guess_remarks_col(tc)
                rmk_col = st.selectbox(
                    f"{region}  MP Remarks / Exclusion column",
                    options=tc,
                    index=_idx(tc, rmk_default),
                    key=f"rmk_{region}",
                    help="The column with exclusion rules: EXCLUDED FROM PROMOTION, MAX 30%, OPEN FOR ALL etc.",
                )

                # Platform VC (optional)
                vc_default = guess_platform_vc_col(tc)
                vc_col = st.selectbox(
                    f"{region}  Platform VC column (optional)",
                    options=["(none)"] + tc,
                    index=_idx(["(none)"] + tc, vc_default),
                    key=f"vc_{region}",
                )
                ...
                if lerr:
                    ...
                    zdata["lookup"] = lookup
                    # Show unique remark values found
                    unique_remarks = lookup["remark"].value_counts()
                    st.caption(
                        f"→ **{len(lookup):,}** articles · "
                        f"**{lookup['RRP'].notna().sum():,}** with RRP · "
                        f"**{(lookup['remark']!='').sum():,}** with remarks"
                    )
                    with st.expander("📋 Remark values found in this ZeCom tab"):
                        st.dataframe(
                            unique_remarks.reset_index()
                            .rename(columns={"index":"Remark","remark":"Count"}),
...
st.caption(f"Active regions: {region_labels}  ·  {date.today().strftime('%d %b %Y')}")

# ── Status row ────────────────────────────────────────────────────────────────
s1, s2, s3 = st.columns(3)
content_ok  = st.session_state["content_df"] is not None
zecom_ok    = any("lookup" in st.session_state["zecom_data"].get(r, {}) for r in active_regions)

with s1:
    st.metric("Content File",
              f"✅ {len(st.session_state['content_df']):,} EANs" if content_ok else "❌ Not uploaded")
with s2:
    loaded = [r for r in active_regions if "lookup" in st.session_state["zecom_data"].get(r, {})]
...
# ─────────────────────────────────────────────────────────────────────────────
# Step: Upload order files
# ─────────────────────────────────────────────────────────────────────────────
st.markdown('<div class="sec">📂 Upload Today\'s Order Files</div>', unsafe_allow_html=True)
st.caption("Upload per marketplace under each region tab. Multiple files per marketplace supported.")
...
for tab, region in zip(region_tabs, active_regions):
    with tab:
        rcolor = REGION_COLORS.get(region, "#333")
        st.markdown(f"**Region: {region}**" +
                    (f"  —  PIC: {pic_names[region]}" if pic_names.get(region) else ""))
...
if st.button("▶️  Run Discount Check", type="primary", use_container_width=True):
    with st.spinner("Mapping EANs → Articles → RRP/SRP → Applying exclusion rules…"):
        combined_lookup = (
    ...
    st.success(f"✅ Processed {len(result):,} orders.")
...
def _kpi(col, lbl, val, fmt="{:.1f}%", cls=""):
    with col:
        st.markdown(
            f'<div class="kpi {cls}"><div class="kpi-v">{fmt.format(val)}</div>'
...
_kpi(k5, "🚨 Flagged",        flagged,
     "{:,}" + f" ({flag_pct:.0f}%)",
     cls="red-kpi" if flagged > 0 else "green-kpi")
_kpi(k6, "RRP Match Rate",   rrp_match,        "{:.1f}%")

# ─────────────────────────────────────────────────────────────────────────────
# TAB LAYOUT — Dashboard tabs
# ─────────────────────────────────────────────────────────────────────────────
st.markdown("")
...
# ════════════════════════════════════════════════════════════════════════
# TAB 1 — EXCLUSION RULE DASHBOARD (Sum RRP / Sum Discount / Total Disc %)
# ════════════════════════════════════════════════════════════════════════
with tab_excl:
    st.markdown("### Exclusion Rule Discount Dashboard")
    st.caption(
        "Shows the **sum of RRP**, **sum of paid price**, **total discount amount** "
        "and **total discount %** for each exclusion remark — across all marketplaces per region."
    )

    excl_df = exclusion_summary(view)

    # ── Overall exclusion table ────────────────────────────────────────────
    st.markdown("#### All Regions — by Exclusion Rule")
    overall_excl = (
        excl_df.groupby(["allowed_rule","flag_severity"]).agg(
            Orders            =("Orders",            "sum"),
            Flagged           =("Flagged",           "sum"),
            Sum_RRP           =("Sum_RRP",           "sum"),
    ...
    overall_excl["Total_Discount_Amt"] = (overall_excl["Sum_RRP"] - overall_excl["Sum_Paid"]).round(2)
    overall_excl["Total_Discount_Pct"] = (
        (overall_excl["Total_Discount_Amt"] / overall_excl["Sum_RRP"]) * 100
    ).round(2)
    overall_excl = overall_excl.sort_values("Total_Discount_Pct", ascending=False).round(2)

    def _colour_sev_row(row):
        sev_colors = {
        ...
        return [sev_colors.get(row["flag_severity"], "")] * len(row)

    styled_overall = (
        overall_excl
        .style
        .apply(_colour_sev_row, axis=1)
        .format({
            "Sum_RRP":             "{:,.2f}",
            "Sum_Paid":            "{:,.2f}",
            "Sum_Seller_Disc":     "{:,.2f}",
    ...
    st.markdown("#### Per Region × Per Marketplace — Exclusion Breakdown")
    for region in active_regions:
        ...
        hdr  = f"**{region}**" + (f"  —  PIC: {pic}" if pic else "")
        st.markdown(hdr)

        # ── Per marketplace sub-tables ─────────────────────────────────────
        ...
        for mptab, mp in zip(mp_tabs, mp_list):
            with mptab:
                ...
                # Mini bar chart — Total Discount % per rule
                if len(mp_excl) > 1:
                    import plotly.express as px
                    fig = px.bar(
                        mp_excl, x="Total_Discount_Pct", y="allowed_rule",
                        orientation="h", color="flag_severity",
                        color_discrete_map=SEVERITY_HEX,
                        labels={"Total_Discount_Pct":"Total Disc %","allowed_rule":"Rule"},
                        title=f"{mp} — Total Discount % by Exclusion Rule",
...
with tab_flag:
    ...
    if flagged_view.empty:
        st.success("✅ No orders are flagged today.")
    else:
        ...
        st.dataframe(
            ...
            .applymap(_sev_colour, subset=["flag_severity"])
...
with tab_mp:
    ...
    # Chart — avg seller disc by marketplace
    try:
        import plotly.express as px
        fig = px.bar(summary, x="marketplace", y="Avg_Seller_Disc",
...
with d2:
    flagged_v = view[view["flagged"]==True]
    if not flagged_v.empty:
        st.download_button(
            "🚨 Flagged Orders CSV",
            data=flagged_v.to_csv(index=False).encode("utf-8"),
...
st.caption(
    ...
    "EXCLUDE = must sell at SRP · MAX X% = seller disc capped · OPEN = no restriction · "
    "Column mapping is fully dynamic — works after every bi-weekly ZeCom update"
)
